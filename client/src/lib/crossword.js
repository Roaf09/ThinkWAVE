...
export function normalizeCrosswordWord(value) {
  return String(value ?? "").trim().toUpperCase().replace(/[^A-Z]/g, "");
}

export function normalizeCrosswordWordKey(value) {
  return normalizeCrosswordWord(value).toLowerCase();
}
...
export function buildCrosswordGrid({ gridSize, words, seed }) {
  const size = Math.min(12, Math.max(5, Number(gridSize) || 8));
  const rand = createSeededRandom(seed);
  const cleaned = Array.from(new Set((words || []).map(normalizeCrosswordWord).filter(Boolean)))
...
export function canFormWordInGrid({ grid, gridSize, word }) {
  const w = normalizeCrosswordWord(word);
  ...
  function dfs(idx, pos, used) {
    ...
    if (normalizeCrosswordWord(grid[idx]) !== w[pos]) return false;
  ...
  for (let start = 0; start < grid.length; start += 1) {
    if (normalizeCrosswordWord(grid[start]) !== w[0]) continue;
...
export function buildCrosswordSignature({ questionId, gridSize, words }) {
  const cleaned = (words || []).map(normalizeCrosswordWordKey).filter(Boolean).sort();
  return JSON.stringify({ questionId: Number(questionId) || 0, gridSize, words: cleaned });
}

export function buildCrosswordSeed(signature) {
  return hashSeed(signature);
}

export function parseCrosswordAnswerList(raw) {
...
export function letterMultiset(word) {
  return normalizeCrosswordWord(word).split("").sort().join("");
}

/** Same word as teacher input: exact match (ignoring case/spacing) or same letters in any order. */
...
export function normalizeWordBank(words) {
  ...
  for (const raw of words || []) {
    const key = normalizeCrosswordWordKey(raw);
    if (!key || seen.has(key)) continue;
    seen.add(key);
    out.push(normalizeCrosswordWord(raw));
  }
  return out;
}

export function resolveCrosswordWordBank({ config = {}, correct = {} }) {
  const cfgAnswers = parseCrosswordAnswerList(config?.answers);
  const corAnswers = parseCrosswordAnswerList(correct?.answers);
...
export function isCrosswordRoundComplete({ foundWords = [], wordBank = [] }) {
  const bank = normalizeWordBank(wordBank);
  const found = new Set((foundWords || []).map(normalizeCrosswordWordKey).filter(Boolean));
  return bank.length > 0 && bank.every((w) => found.has(normalizeCrosswordWordKey(w)));
}
...
export function validatePathSpellsWord({ grid, gridSize, path, word }) {
  ...
  for (let i = 0; i < path.length; i += 1) {
    ...
    if (normalizeCrosswordWord(grid[idx]) !== w[i]) return false;
...
/** Bookworm-style: clear used tiles, drop letters down, refill from top. */
...
export function computeCrosswordPoints(wordLength, config = {}, basePoints = 1) {
...
export function loadCrosswordGridState({ config, correct, questionId: _questionId, priorPayload }) {
  ...
  if (Array.isArray(prior.grid) && prior.grid.length === gridSize * gridSize) {
    return {
      grid: prior.grid.map((ch) => normalizeCrosswordWord(ch) || ch),
      gridSize,
      wordBank,
  ...
  if (Array.isArray(config?.grid) && config.grid.length === gridSize * gridSize) {
    return {
      grid: config.grid.map((ch) => normalizeCrosswordWord(ch) || ch),
      gridSize,
      wordBank,
      refillCounter: 0,
      streak: 0,
  ...
  const signature = `${buildCrosswordSignature({ questionId: 0, gridSize, words: wordBank })}-${Number(config?.gridSeed || 0)}`;
  const seed = buildCrosswordSeed(signature);
  const built = buildCrosswordGrid({ gridSize, words: wordBank, seed });
  return {
    grid: built.grid,
...
export function getPathLinePoints(path, gridSize, cellSize, gap) {
  const n = Number(gridSize) || 0;
  const size = Number(cellSize) || 48;
  ...
  return (path || []).map((idx) => {
    ...
    const c = i % n;
    return { x: c * step + size / 2, y: r * step + size / 2 };
  });
}
