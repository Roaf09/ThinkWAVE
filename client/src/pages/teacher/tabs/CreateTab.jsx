...
export default function CreateTab({ guestMode = false, tutorial }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!newId) {
        setMsg("Quiz was created but the server did not return its id. Please try again.");
        return;
      }
      // Confirm the new quiz is readable with this login before leaving the
      // page — otherwise the builder opens on a dead id ("Quiz not found").
      try {
        await api.get(`/quizzes/${newId}`);
      } catch (verifyErr) {
        if (verifyErr?.response?.status === 404) {
          setMsg(`Quiz #${newId} was created but can't be reloaded with this login. Your session may have changed (another tab/account or a database reset) — please log out and log in again, then retry.`);
        } else {
          setMsg(verifyErr?.response?.data?.message || "Could not open the new quiz. Please try again.");
        }
        return;
      }
  ...
  return <div className="container grid gap-[20px]">
    ...
    <section className="w-full box-border" style={card(c)}>
      <form onSubmit={handleSubmit} className="grid gap-[22px]">
        ...
        {msg && <div className="px-[14px] py-[12px] rounded-[14px] text-[13px]" style={{ background: c.redBg, border: `1px solid ${c.redBorder}`, color: c.redFg }}>{msg}</div>}
      </form>
    </section>

    <section className="grid gap-[12px]">
      <div><div className="font-[900] text-[17px]" style={{ color: c.text }}>Recent template shortcuts</div><div className="text-[13px] mt-[6px]" style={{ color: c.textMuted }}>Reuse a recent structure to start faster.</div></div>
