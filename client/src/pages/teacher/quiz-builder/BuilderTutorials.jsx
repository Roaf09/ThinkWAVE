    ...
    <>
      ...
      {!guestMode && !modifiedTutorialOpen && builderTutorialStage === "intro" && (
        <ThinkBotTutorial accentColor={quiz ? templateAccent(quiz.template_type) : undefined} placement="center" dialogWidth={430} clickAnywhere onClickAnywhere={() => setBuilderTutorialStage("question")}>
          <p>Every template has its own builder tools, so this walkthrough is just for <strong>{quiz ? ({ MCQ: "Multiple Choice", TRUE_FALSE: "True or False", TYPE_ANSWER: "Identification", MATCHING: "Matching", GUESS_WORD_4PICS: "Guess Word", CROSSWORD: "Crossword" }[normalizeTemplateType(quiz.template_type)] || quiz.template_type) : "this template"}</strong>.</p>
        </ThinkBotTutorial>
      )}
      ...
      {!guestMode && !modifiedTutorialOpen && builderTutorialStage === "crossword_word_controls" && normalizeTemplateType(quiz?.template_type) === "CROSSWORD" && <ThinkBotTutorial
