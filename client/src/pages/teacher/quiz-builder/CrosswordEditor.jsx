...
/**
 * CrosswordEditor — isolated sub-component so useState for rawText is
 ...
 */
export function CrosswordEditor({ cor, cfg, onChange, ui, c, maxWords = null, isMobile = false }) {
  ...
  function buildSavedGrid(seedValue) {
    const signature = `${buildCrosswordSignature({ questionId: 0, gridSize, words: normalized })}-${seedValue}`;
    return buildCrosswordGrid({ gridSize, words: normalized, seed: buildCrosswordSeed(signature) });
  }
