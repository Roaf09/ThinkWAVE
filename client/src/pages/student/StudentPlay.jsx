...
export default function StudentPlay() {
  ...
  const countdown = useMemo(() => {
    if (state?.status !== "LIVE" || !state?.question_started_at) return 0;
    const startedAtMs = new Date(state.question_started_at).getTime();
    if (!Number.isFinite(startedAtMs)) return 0;
    const serverNowMs = nowMs - clockOffsetMs;
    return Math.max(0, Math.ceil((startedAtMs - serverNowMs) / 1000));
  }, [state?.status, state?.question_started_at, nowMs, clockOffsetMs]);
