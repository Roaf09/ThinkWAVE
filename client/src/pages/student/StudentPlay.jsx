...
export default function StudentPlay() {
  ...
  useEffect(() => {
    if (state?.status !== "LIVE" || !state?.question_started_at) {
      setCountdown(null);
      return undefined;
    }
    ...
    const updateCountdown = () => {
      ...
      const remainingSec = Math.max(0, Math.ceil((startedAtMs - serverNowMs) / 1000));
      setCountdown(remainingSec);
    };

    updateCountdown();
    const iv = setInterval(updateCountdown, 200);
    return () => clearInterval(iv);
  }, [state?.status, state?.current_question_index, state?.question_started_at, clockOffsetMs]);
