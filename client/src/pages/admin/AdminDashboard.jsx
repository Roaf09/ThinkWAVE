...
function Metric({label,value,icon,tone="#2b6cff"}){const c=useColors();const {dark}=useTheme();const palette=metricPalette(tone,dark);const ink=dark?"#ffffff":"#0f172a";return <div className={`tw-admin-card tw-admin-metric tw-tone-${icon}`} style={{...card(c),display:"flex",alignItems:"center",gap:14,padding:15,"--metric-tone":palette.background,"--metric-border":palette.border,"--metric-ink":ink,border:`4px solid ${palette.border}`,background:palette.background,color:ink}}><span style={{width:44,height:44,borderRadius:14,display:"grid",placeItems:"center",background:dark?"rgba(255,255,255,.15)":"rgba(255,255,255,.52)",color:ink,border:dark?"1px solid rgba(255,255,255,.24)":"1px solid rgba(15,23,42,.15)"}}><TwIcon name={icon} size={22}/></span><div><b style={{display:"block",fontSize:27,color:ink}}>{Number(value||0).toLocaleString()}</b><small style={{color:ink,fontWeight:850,opacity:.86}}>{label}</small></div></div>}
...
function Institution(){
  ...
  return <div className="container"><Heading title="Institution"/>{error&&<ApiMessage text={error}/>} {loading?<LoadingCard text="Loading institution dashboard…"/>:<>
    <div className="tw-admin-institution-v251">
      <section style={card(c)} className="tw-admin-invite-card tw-admin-institution-panel"><ChartTitle title="Teacher Invitation" icon="invitation"/>{invite?<><div style={{...quiet(c),textAlign:"center",padding:24,background:"linear-gradient(135deg,rgba(43,108,255,.18),rgba(59,130,246,.08))"}}><small style={{color:c.textMuted}}>Active code</small><div className="tw-admin-invite-code" style={{color:c.accent}}>{invite.invite_code}</div></div><div className="tw-admin-invite-actions"><AdminPressButton tone="blue" onClick={generate}>Regenerate</AdminPressButton><AdminPressButton tone="red" onClick={revoke}>Revoke</AdminPressButton></div>{notice&&<div className="tw-admin-invite-notice" style={{...quiet(c),color:c.greenFg,borderColor:c.greenBorder}}>{notice}</div>}</>:<div style={{...quiet(c),textAlign:"center",padding:28}}><p style={{color:c.textMuted}}>No active invitation code yet.</p><AdminPressButton tone="blue" onClick={generate}>Generate Code</AdminPressButton>{notice&&<div className="tw-admin-invite-notice" style={{color:c.greenFg}}>{notice}</div>}</div>}</section>
