...
export default function Landing(){
  ...
  const [slide,setSlide]=useState(0); const [starOpacity,setStarOpacity]=useState(1); const [templatePreview,setTemplatePreview]=useState(null); const [previewClosing,setPreviewClosing]=useState(false); const hoverTimer=useRef(null); const closeTimer=useRef(null);
  useEffect(()=>{
  ...
  useEffect(()=>{const onScroll=()=>setStarOpacity(Math.max(0,1-window.scrollY/900));onScroll();window.addEventListener("scroll",onScroll,{passive:true});return()=>window.removeEventListener("scroll",onScroll)},[]);
  ...
  return <div id="home" className="tw-landing-page tw-starry-page" style={{"--tw-star-opacity":starOpacity,background:c.pageBg,color:c.text}}>
    ...
    <div className="tw-back-top-wrap"><button type="button" className="tw-back-top" aria-label="Back to top" onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{background:c.cardBg3,color:c.text,borderColor:c.border}}><TwIcon name="chevronUp" size={21}/></button></div>
