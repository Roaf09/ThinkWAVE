...
const templateCopy = {
  MCQ:"Create quick checks with one or two correct answers.", TRUE_FALSE:"Check understanding with simple, clear choices.",
...
export default function Landing(){
  ...
  return <div id="home" className="tw-landing-page" style={{background:c.pageBg,color:c.text}}>
    ...
    <main>
      <section className="tw-landing-hero">
        <div className="tw-landing-hero-copy">
          <div className="tw-eyebrow">Playful learning. Clear progress.</div>
          <h1>Turn every lesson into a learning experience students want to join.</h1>
          <div className="tw-landing-cta"><button className="tw-cta-primary tw-landing-highlight-action" onClick={getStarted} disabled={checking}>{checking?"Please wait…":"Get Started"}</button>{!isFirstRun&&<button className="tw-cta-secondary tw-landing-highlight-action" style={{color:c.text,borderColor:c.accent}} onClick={()=>nav("/login?role=admin")}>Join as Admin</button>}</div>
        </div>
        <div className="tw-landing-hero-panel" style={{background:c.cardBg,borderColor:c.border}}>
          <button onClick={()=>scroll("templates")} className="tw-hero-link-card" style={{background:c.cardBg2,borderColor:c.border,color:c.text}}><IconBubble name="mcq" c={c}/><div><b>Templates</b><small>Choose an activity that fits your lesson.</small></div><TwIcon name="arrow"/></button>
          <button onClick={()=>scroll("analytics")} className="tw-hero-link-card" style={{background:c.cardBg2,borderColor:c.border,color:c.text}}><IconBubble name="chart" c={c}/><div><b>Analytics</b><small>Turn classroom results into useful insight.</small></div><TwIcon name="arrow"/></button>
          <div className="tw-hero-visual tw-hero-visual-compact" aria-hidden="true">
            <div className="tw-hero-row">
              <IconBubble name="live" c={c} size={38} iconSize={19} />
              <div style={{flex:1}}>
                <div style={{height:9,borderRadius:99,background:dark?"rgba(255,255,255,.14)":"rgba(43,108,255,.14)",marginBottom:8}}><div className="tw-hero-bar"/></div>
                <div style={{height:8,width:"58%",borderRadius:99,background:dark?"rgba(255,255,255,.12)":"rgba(43,108,255,.12)"}}/>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="tw-rounded-section" style={{background:dark?"rgba(255,255,255,.035)":"rgba(255,255,255,.55)"}}>
        <div className="tw-section-intro tw-why-heading"><h2>Why ThinkWAVE?</h2><p className="tw-three-line-copy"><span>Make preparation lighter.</span><span>Participation stronger.</span><span>Classroom decisions more informed.</span></p></div>
        <div className="tw-benefit-grid">{benefits.map(([icon,title,copy])=><article key={title} className="tw-hover-card" style={{background:c.cardBg,borderColor:c.border}}><IconBubble name={icon} c={c} size={50}/><h3>{title}</h3><p style={{color:c.textMuted}}>{copy}</p></article>)}</div>
      </section>

      <section id="templates" className="tw-flat-section"><div className="tw-centered-heading"><span className="tw-eyebrow">Designed for different ways of learning</span><h2>Our Templates</h2><p style={{color:c.textMuted}}>Pick a format, add your content, and make each classroom activity feel fresh.</p></div>
        <div className="tw-template-grid">{Object.entries(TEMPLATE_PALETTES).map(([key,t])=><article key={key} className="tw-template-landing-card" style={{background:`linear-gradient(145deg,${t.accent}20,${c.cardBg})`,borderColor:`${t.accent}70`}}><span style={{background:`${t.accent}24`,color:t.accent}}><TwIcon name={t.icon} size={26}/></span><h3 style={{color:t.accent}}>{t.label}</h3><p style={{color:c.textMuted}}>{templateCopy[key]}</p></article>)}</div>
      </section>

      <section id="analytics" className="tw-rounded-section tw-analytics-section" style={{background:dark?"rgba(255,255,255,.035)":"rgba(255,255,255,.55)"}}>
        <div><span className="tw-eyebrow">Results you can act on</span><h2>See the learning behind every answer.</h2><p style={{color:c.textMuted}}>See who joined, how the class performed, and which questions may need another look. Schools can also explore more detailed insights and download reports whenever they need them.</p></div>
        <div className="tw-analytics-visual" style={{background:c.cardBg,borderColor:c.border}}><div className="tw-analytics-metrics"><div><small>Average</small><b>84%</b></div><div><small>Submitted</small><b>42</b></div><div><small>Highest</small><b>98</b></div></div><div className="tw-analytics-chart">{[42,66,54,84,72,94].map((v,i)=><span key={i} style={{height:`${v}%`}}/>)}</div><div className="tw-plan-badges"><span>Basic overview</span><span>Institution insights</span></div></div>
      </section>
    ...
    {modalOpen&&<><div className="tw-modal-backdrop" onClick={()=>setModalOpen(false)}/><div className="tw-start-modal" style={{background:c.cardBg3,borderColor:c.border,color:c.text}}><button className="tw-modal-x" onClick={()=>setModalOpen(false)}><TwIcon name="close"/></button>{modalStep!=="root"&&<button className="tw-modal-back" onClick={()=>{setModalMsg("");setModalStep("root")}}>← Back</button>}<h2>{modalTitle}</h2><p style={{color:c.textMuted}}>{modalStep==="root"?"Choose how you want to enter ThinkWAVE.":modalStep==="joinCode"?"Enter the live session code shared by the host.":"Choose the option that fits you."}</p>
      {modalStep==="root"&&<div className="tw-modal-options"><button onClick={()=>setModalStep("joinRole")}><IconBubble name="join" c={c}/><b>Join Session</b><small>Enter a live activity</small></button><button onClick={()=>setModalStep("hostRole")}><IconBubble name="host" c={c}/><b>Host Session</b><small>Create and lead an activity</small></button></div>}
      {modalStep==="joinRole"&&<div className="tw-modal-options"><button onClick={()=>chooseJoin("guest")}><IconBubble name="guest" c={c}/><b>Guest Join</b><small>Join without an account</small></button><button onClick={()=>chooseJoin("student")}><IconBubble name="student" c={c}/><b>Student</b><small>Use your student account</small></button></div>}
      {modalStep==="hostRole"&&<div className="tw-modal-options"><button onClick={()=>{setModalOpen(false);nav("/login")}}><IconBubble name="teacher" c={c}/><b>Teacher</b><small>Open your teacher workspace</small></button><button onClick={()=>{setModalOpen(false);nav("/guest")}}><IconBubble name="guest" c={c}/><b>Guest Host</b><small>Try a temporary workspace</small></button></div>}
      {modalStep==="joinCode"&&<form onSubmit={continueJoin} className="tw-code-form"><input value={joinCode} onChange={e=>setJoinCode(e.target.value.toUpperCase())} placeholder="SESSION CODE" autoFocus style={{background:c.inputBg,color:c.text,borderColor:c.inputBorder}}/>{modalMsg&&<div className="tw-form-error">{modalMsg}</div>}<button className="tw-cta-primary">Continue</button></form>}
    </div></>}
  </div>
}
