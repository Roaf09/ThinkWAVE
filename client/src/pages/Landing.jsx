...
export default function Landing(){
  ...
  return <div id="home" className="tw-landing-page tw-starry-page" style={{"--tw-star-opacity":starOpacity,background:c.pageBg,color:c.text}}>
    ...
    {modalOpen&&createPortal(<><div className="tw-modal-backdrop" onClick={()=>setModalOpen(false)}/><div className="tw-start-modal" style={{background:c.cardBg3,borderColor:c.border,color:c.text}}><button className="tw-modal-x" onClick={()=>setModalOpen(false)}><TwIcon name="close"/></button>{modalStep!=="root"&&<button className="tw-modal-back" onClick={()=>{setModalMsg("");setModalStep("root")}}>Back</button>}<h2>{modalTitle}</h2><p style={{color:c.textMuted}}>{modalStep==="root"?"Choose how you want to enter ThinkWAVE.":modalStep==="joinCode"?"Enter the live session code shared by the host.":"Choose the option that fits you."}</p>{modalStep==="root"&&<div className="tw-modal-options"><button onClick={()=>setModalStep("joinRole")}><IconBubble name="join" c={c}/><b>Join Session</b><small>Enter a live activity</small></button><button onClick={()=>setModalStep("hostRole")}><IconBubble name="host" c={c}/><b>Host Session</b><small>Create and lead an activity</small></button></div>}{modalStep==="joinRole"&&<div className="tw-modal-options"><button onClick={()=>chooseJoin("guest")}><IconBubble name="guest" c={c}/><b>Guest Join</b></button><button onClick={()=>chooseJoin("student")}><IconBubble name="student" c={c}/><b>Student</b></button></div>}{modalStep==="hostRole"&&<div className="tw-modal-options"><button onClick={()=>{setModalOpen(false);nav("/login")}}><IconBubble name="teacher" c={c}/><b>Teacher</b></button><button onClick={()=>{setModalOpen(false);nav("/guest")}}><IconBubble name="guest" c={c}/><b>Guest Host</b></button></div>}{modalStep==="joinCode"&&<form onSubmit={continueJoin} className="tw-code-form"><input value={joinCode} onChange={e=>setJoinCode(e.target.value.toUpperCase())} placeholder="SESSION CODE" autoFocus style={{background:c.inputBg,color:c.text,borderColor:c.inputBorder}}/>{modalMsg&&<div className="tw-form-error">{modalMsg}</div>}<button className="tw-cta-primary">Continue</button></form>}</div></>,document.body)}
  </div>
}
