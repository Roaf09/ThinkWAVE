...
export default function Landing(){
  ...
  const [modalOpen,setModalOpen]=useState(false); const [modalStep,setModalStep]=useState("root"); const [entryMode,setEntryMode]=useState(""); const [joinCode,setJoinCode]=useState(""); const [modalMsg,setModalMsg]=useState(""); const [checking,setChecking]=useState(false); const [codeChecking,setCodeChecking]=useState(false);
  ...
  async function continueJoin(e){e.preventDefault();const code=joinCode.trim().toUpperCase();if(code.length<4)return setModalMsg("Please enter a valid session code first.");if(codeChecking)return;setCodeChecking(true);setModalMsg("");try{await api.post("/sessions/validate-code",{code});setModalOpen(false);nav(`/play?code=${encodeURIComponent(code)}&entry=${encodeURIComponent(entryMode||"guest")}`)}catch(err){setModalMsg(err?.response?.data?.message||"Invalid code / session not active")}finally{setCodeChecking(false)}}
  ...
  return <div id="home" className="tw-landing-page tw-starry-page" style={{"--tw-star-opacity":starOpacity,background:c.pageBg,color:c.text}}>
    ...
    {modalOpen&&createPortal(<><div className="tw-modal-backdrop" onClick={()=>setModalOpen(false)}/><div className="tw-start-modal" style={{background:c.cardBg3,borderColor:c.border,color:c.text}}><button className="tw-modal-x" onClick={()=>setModalOpen(false)}><TwIcon name="close"/></button>{modalStep!=="root"&&<button className="tw-modal-back" onClick={()=>{setModalMsg("");setModalStep("root")}}>Back</button>}<h2>{modalTitle}</h2><p style={{color:c.textMuted}}>{modalStep==="root"?"Choose how you want to enter ThinkWAVE.":modalStep==="joinCode"?"Enter the live session code shared by the host.":"Choose the option that fits you."}</p>{modalStep==="root"&&<div className="tw-modal-options"><button className="tw-role-option tw-role-option-red" onClick={()=>setModalStep("joinRole")}><IconBubble name="join" c={c} tone="red"/><b>Join Session</b><small>Enter a live activity</small></button><button className="tw-role-option tw-role-option-blue" onClick={()=>setModalStep("hostRole")}><IconBubble name="host" c={c} tone="blue"/><b>Host Session</b><small>Create and lead an activity</small></button></div>}{modalStep==="joinRole"&&<div className="tw-modal-options"><button className="tw-role-option tw-role-option-yellow" onClick={()=>chooseJoin("guest")}><IconBubble name="guest" c={c} tone="yellow"/><b>Guest Join</b></button><button className="tw-role-option tw-role-option-red" onClick={()=>chooseJoin("student")}><IconBubble name="student" c={c} tone="red"/><b>Student</b></button></div>}{modalStep==="hostRole"&&<div className="tw-modal-options"><button className="tw-role-option tw-role-option-blue" onClick={()=>{setModalOpen(false);nav("/login")}}><IconBubble name="teacher" c={c} tone="blue"/><b>Teacher</b></button><button className="tw-role-option tw-role-option-green" onClick={()=>{setModalOpen(false);nav("/guest")}}><IconBubble name="guest" c={c} tone="green"/><b>Guest Host</b></button></div>}{modalStep==="joinCode"&&<form onSubmit={continueJoin} className="tw-code-form"><input value={joinCode} onChange={e=>setJoinCode(e.target.value.toUpperCase())} placeholder="SESSION CODE" autoFocus style={{background:c.inputBg,color:c.text,borderColor:c.inputBorder}}/>{modalMsg&&<div className="tw-form-error">{modalMsg}</div>}<button className="tw-auth-primary p-[14px] rounded-[14px] border-[3px] border-brand bg-brand text-white text-base font-extrabold cursor-pointer shadow-[0_10px_24px_rgba(43,108,255,0.25)] disabled:opacity-70" disabled={codeChecking}>{codeChecking?"Checking…":"Continue"}</button></form>}</div></>,document.body)}
  </div>
}
